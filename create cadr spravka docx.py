...
docx_file = 'Кадры 44.03.01 ПО АЯ 2018 гнп.docx'

word_template = r'C:\Users\User\Desktop\Рабочая папка\от Геращенковой\2023\февраль 2023\КС_ГиСД' + '\\' + docx_file

excel_file = r'+ГиСД_КС_2019_очное.xlsx'

excel_path = r'C:\Users\User\Desktop\Рабочая папка\от Геращенковой\2023\февраль 2023\КС_ГиСД' + '\\' + excel_file
...
for name_opop in names_opop:
        df_pivot_table = pd.pivot_table(df, index=['Общий_код', 'Год_набора', 'Форма_обучения', 'Номер', 'Дисциплина_практика_ГИА', 'ФИО_итоговые', 
        'Условия_привлечения', 'Степень_звание', 'процент_занимается наукой', 'Процент_практиков', 'Процент_остепененности', 'Уровень_образования', 'Доп_образование'], 
        ...
        level_opop = pd.Series(df['Уровень_ВО']).drop_duplicates().to_string().replace('0    ', '')

        code_opop = pd.Series(df['Код_направление']).drop_duplicates().to_string().replace('0    ', '')

        profile_opop = pd.Series(df['Профиль_подготовки_специализация']).drop_duplicates().to_string().replace('0    ', '')
        ...
        print(level_opop)
        print(profile_opop)
        print(perc_all, perc_os, perc_pr)
        ...
        output_path = r'C:\Users\User\Desktop\Рабочая папка\от Геращенковой\2023\февраль 2023\КС_ГиСД' +  '\\' + f"КС_{name_opop}.docx"
